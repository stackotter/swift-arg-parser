...
- You can now use StackOtterArgParser with Musl libc. ([#574])
...
- Fixes a bug where non-optional properties in ParsableCommands could cause
  StackOtterArgParser to crash. You can now include properties like dictionaries in
...
- Invalid `init(from:)` decoding initializers are now correctly diagnosed by
  StackOtterArgParser's validators. ([#487])
...
- `StackOtterArgParser` now includes a SwiftPM plugin for generating `man` pages.
...
- `StackOtterArgParser` now requires Swift 5.5.
...
- When a user provides an incorrect value for an option, an 
  `StackOtterArgParser`-based program now includes the valid values when possible.
...
- Resolves issues that prevented building `StackOtterArgParser` for WebAssembly using
...
`StackOtterArgParser` is now source stable!

### Changes

- `StackOtterArgParser` now provides a DocC documentation catalog, so you
...
- Command-line tools built with `StackOtterArgParser` now include an experimental
...
- The parser provides near-miss suggestions when a user provides an unknown
  option.
- `StackOtterArgParser` now builds on Windows.
...
- `StackOtterArgParser` initial release.
